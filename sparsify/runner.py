...
"""Matryoshka-style runner that applies top-k selection
separately to different slices of the latent space.

**How it works:**
1. For each slice size k_i, computes top-k selection on the corresponding subset
   of the latent space
...
- No need to materialize pre_acts in
the runner (computed only when needed in SparseCoder)
...
class MatryoshkaRunner:
    """Matryoshka-style runner that processes different
    slices of the latent space independently.

    This runner applies top-k selection separately to different subsets of the latent
    space, computes losses for each slice, and aggregates them. It's designed for
    scenarios where you want to analyze different granularities of feature selection
    simultaneously.
    """
    ...
    def __init__(self):
        ...
        Attributes:
            outputs: Maps module names to their MidDecoder objects for cross-layer
                coalescing
            to_restore: Tracks MidDecoder objects that need gradient restoration
                (rarely used now)
        """
    ...
    def _matryoshka_sizes(mid: MidDecoder) -> List[int]:
        ...
        2. matryoshka_expansion_factors: Multipliers applied to input dimension
           [1.0, 2.0, 4.0, ...]
    ...
    def _decode_slice(
        ...
        **kwargs,
        ...
        This method handles the cross-layer coalescing logic inherited from
        CrossLayerRunner. It combines activations and indices from multiple layers,
        applies top-k selection if enabled, and produces the final decoded output.
        ...
        Args:
            ...
            detach_grad: Whether to detach gradients (largely unused in this
                implementation)
        ...
        if mid_out.sparse_coder.cfg.do_coalesce_topk:
            ...
            if mid_out.sparse_coder.cfg.topk_coalesced:
                if isinstance(candidate_values, DTensor):
                    ...
                    def mapper(values, indices):
                        best_vals, best_idxs = torch.topk(
                            values, k=mid_out.sparse_coder.cfg.k, dim=1
                        )
            ...
            if mid_out.sparse_coder.cfg.coalesce_topk == "concat":
                ...
                new_mid_out = mid_out.copy(
                    indices=best_indices, activations=best_values
                )
                ...
                for i, layer_mid in enumerate(layer_mids):
                    ...
                    new_mid_out = layer_mid.copy(
                        indices=best_indices_local, activations=best_values_local
                    )
                    out = new_mid_out(
                        y,
                        layer_mid.index - 1,
                ...
                raise ValueError(
                    "Unknown coalesce_topk mode: "
                    + mid_out.sparse_coder.cfg.coalesce_topk
                )
    ...
    def decode(
        ...
        **kwargs,
        ...
        total_fvu = sum(
            slice_result.fvu for slice_result in matryoshka_output.slice_results
        )
        total_aux = sum(
            slice_result.auxk_loss for slice_result in matryoshka_output.slice_results
        )
        total_multi = sum(
            slice_result.multi_topk_fvu
            for slice_result in matryoshka_output.slice_results
        )
        ...
        print(
            f"  Non-zero activations: {non_zero_count:,} / {total_elements:,} "
            f"({100-sparsity:.1f}%)"
        )
        print(
            f"  Zero activations: {zero_count:,} / {total_elements:,} ({sparsity:.1f}%)"
        )
        print(
            f"  Activation range: {final_acts.min().item():.6f} to "
            f"{final_acts.max().item():.6f}"
        )
    ...
    def restore(self):
        ...
        # since slice computation happens in SparseCoder and we only create one
        # MidDecoder.
